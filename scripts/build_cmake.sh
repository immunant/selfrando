...
mkdir -p $BUILD_DIR
cd $BUILD_DIR
cmake $SR_DIR -DCMAKE_INSTALL_PREFIX:PATH="$BUILD_DIR" -DCMAKE_BUILD_TYPE=Release -DSR_ARCH=$SR_ARCH -DBUILD_SHARED_LIBS=1 -DSR_FORCE_INPLACE=1 -G Ninja $CMAKE_ARGS "$@"
ninja $NINJA_ARGS
ninja install
